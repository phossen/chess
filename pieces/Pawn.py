        ...
        if self.color == Color.WHITE:
            ...
            if position[1] not in board.board[chr(ord(position[0]) + 1)]:
                legal_positions.append(
                    (chr(ord(position[0]) + 1), position[1]))
            if position[1] != "a":
                if chr(ord(position[1]) -
                       1) in board.board[chr(ord(position[0]) + 1)]:
                    if board.board[chr(ord(position[0]) +
                                       1)][chr(ord(position[1]) -
                                               1)].color == Color.BLACK:
                        legal_positions.append(
                            (chr(ord(position[0]) + 1), chr(ord(position[1]) - 1)))
            if position[1] != "h":
                if chr(ord(position[1]) +
                       1) in board.board[chr(ord(position[0]) + 1)]:
                    if board.board[chr(ord(position[0]) +
